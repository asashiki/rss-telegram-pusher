...
RSSHUB_TRAILING_TIMESTAMP_REGEX = re.compile(
...
def extract_description(entry):
    ...
    if raw_description:
        ...
        cleaned = re.sub(r"<\s*(br|hr)\b[^>]*>", "\n", cleaned, flags=re.IGNORECASE)
        cleaned = re.sub(r"</\s*(p|div|blockquote|small)\s*>", "\n", cleaned, flags=re.IGNORECASE)
        cleaned = re.sub(r"<[^>]+>", "", cleaned)
        cleaned = cleaned.replace("\r", "").strip()
        cleaned = RSSHUB_TRAILING_TIMESTAMP_REGEX.sub("", cleaned).strip()
        cleaned = re.sub(r"\n{3,}", "\n\n", cleaned)
        if cleaned:
            return cleaned
...
def build_telegram_html(text, link=None, limit=TELEGRAM_MESSAGE_LIMIT, prefix=None, timestamp=None):
    ...
    suffix = ""
    while True:
        escaped_body = html.escape(f"{prefix}{raw_text}{suffix}")
